...
public class AudioManager implements AudioBus {
  ...
  private final java.util.List<ManagedSource> usedSources = new java.util.ArrayList<>();
  ...
  volatile AudioStream musicStream;
  ...
  public void updateMusicFade() {
    recycleStoppedSources();
    AudioStream stream = musicStream;
    if (!musicFading || stream == null) return;
    ...
    stream.setGain(musicFadeCurrent);
  }
  ...
  private ManagedSource acquireSource() {
    ...
    if (!freeSources.isEmpty()) {
      source = freeSources.pollFirst();
    } else if (totalSources < POOL_MAX) {
      totalSources++;
      source = createSource();
    }
    if (source != null) {
  ...
  private void recycleStoppedSources() {
    for (int i = usedSources.size() - 1; i >= 0; i--) {
      ManagedSource source = usedSources.get(i);
      int state = alGetSourcei(source.id, AL_SOURCE_STATE);
      if (state == AL_STOPPED || state == AL_INITIAL) {
        alSourceStop(source.id);
        alSourcei(source.id, AL_BUFFER, 0);
        usedSources.remove(i);
