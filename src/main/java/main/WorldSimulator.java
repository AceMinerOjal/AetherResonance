...
public class WorldSimulator {
  ...
  public void simulate(double dt) {
    ...
    for (int i = 0; i < enemies.size(); i++) {
      ...
      enemy.update(dt, map, players, enemies);
  ...
  private void ensureEnemiesForCurrentMap(TiledMap map) {
    ...
    if (currentMapId.equals(enemyMapId)) {
      return;
    }
  ...
  private void awardExp(Enemy enemy) {
    int exp = enemy.getExpReward();
    if (exp <= 0) return;
    double rangeSq = GameConfig.DEFAULT.xp().awardRangePx();
    rangeSq *= rangeSq;
