...
final class NetProtocol {
  ...
  public static ParsedRestoreMessage parseRestoreMessage(String msg) {
    ...
    try {
      ...
      hp = Double.parseDouble(parts[7]);
      mana = Double.parseDouble(parts[8]);
      ap = Double.parseDouble(parts[9]);
      defence = Double.parseDouble(parts[10]);
      level = Integer.parseInt(parts[11]);
      exp = Integer.parseInt(parts[12]);
    } catch (NumberFormatException e) {
      return null;
    }
