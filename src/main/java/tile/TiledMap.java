...
public class TiledMap {
  ...
  public static final class Tileset {
    ...
    public Tileset(int firstGid, int tileWidth, int tileHeight, int columns,
        int tileCount, int margin, int spacing, BufferedImage image, String imagePath, Set<Integer> solidLocalTileIds) {
  ...
  public List<SpawnPoint> getEnemySpawnPoints() {
    ...
    for (int li = 0; li < layers.size(); li++) {
      ...
      int layerCount = 0;
      for (int i = 0; i < tiles.size() && layerCount < MAX_PER_LAYER; i += step) {
        int[] t = tiles.get(i);
