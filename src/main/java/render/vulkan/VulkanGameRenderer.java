...
public final class VulkanGameRenderer {
  ...
  private static final float ELEMENT_BAR_WIDTH = 16f;
  private static final float ELEMENT_BAR_HEIGHT = 2f;
  private static final float ELEMENT_BAR_Y_OFFSET = -4f;
  ...
  public List<RenderCommand> buildLocalCommands(
  ...
  public List<RenderCommand> buildRemoteCommands(TiledMap map, NetSnapshot snapshot, float camX, float camY) {
    ensureTilesetTexturesLoaded(map);
  ...
  private void buildTileCommands(List<RenderCommand> commands, TiledMap map, float camX, float camY) {
    if (map == null) return;
    ...
    for (int li = 0; li < map.getLayerCount(); li++) {
      ...
      for (Map.Entry<Integer, List<float[]>> entry : batch.entrySet()) {
        ...
        commands.add(new RenderCommand(verts, ti, z));
      }
    }
  }
  ...
  private void appendPlayerCommand(List<RenderCommand> commands, PlayerVisual player) {
    int z = 1000;

    if (player.appearanceId().isBlank()) {
      commands.add(RenderCommand.rect(
    ...
    if (player.direction() == Entity.Direction.LEFT) {
      commands.add(RenderCommand.texQuad(
          player.x(), player.y(), player.width(), player.height(),
    ...
    appendElementIndicator(commands, player, z + 1);
  }

  private void appendElementIndicator(List<RenderCommand> commands, PlayerVisual player, int z) {
    if (player.element() == null) return;
    float[] rgb = elementColor(player.element());
    float cx = player.x() + player.width() / 2f - ELEMENT_BAR_WIDTH / 2f;
    float cy = player.y() + player.height() + ELEMENT_BAR_Y_OFFSET;
    commands.add(RenderCommand.rect(
        cx, cy, ELEMENT_BAR_WIDTH, ELEMENT_BAR_HEIGHT,
        rgb[0], rgb[1], rgb[2], WHITE, z));
  }

  private static float[] elementColor(SignatureElement element) {
    return switch (element) {
      case FIRE -> new float[]{1.0f, 0.3f, 0.1f};
      case ICE -> new float[]{0.4f, 0.8f, 1.0f};
      case LIGHTNING -> new float[]{1.0f, 0.9f, 0.2f};
      case EARTH -> new float[]{0.7f, 0.5f, 0.2f};
      case WIND -> new float[]{0.3f, 0.9f, 0.7f};
      case SHADOW -> new float[]{0.6f, 0.2f, 0.8f};
    };
  }

  // -- Enemy command --

  private void appendEnemyCommand(List<RenderCommand> commands, EnemyVisual enemy) {
    int z = 1001;
    ...
    if (texSlot != null) {
      renderTexturedEnemy(commands, enemy, texSlot, z);
      return;
    }

    renderFallbackEnemy(commands, enemy, z);
  }

  private void renderTexturedEnemy(List<RenderCommand> commands,
      EnemyVisual enemy, int texSlot, int z) {
    SpriteUv uv = resolveSpriteUv(WALK_LAYOUT, enemy.animation(), enemy.direction(), enemy.frame());
    float u0 = enemy.direction() == Entity.Direction.LEFT ? uv.u1() : uv.u0();
    float u1 = enemy.direction() == Entity.Direction.LEFT ? uv.u0() : uv.u1();
    commands.add(RenderCommand.texQuad(
  ...
  private void renderFallbackEnemy(List<RenderCommand> commands,
      EnemyVisual enemy, int z) {
    float r, g, b;
    if (enemy.variant == 0) {
  ...
  private SpriteUv resolveSpriteUv(
    ...
    int maxFrame = layout.cols() - 1;
    int clampedFrame = Math.min(frame, maxFrame);
    float u0 = (float) (clampedFrame * layout.frameWidth()) / layout.sheetWidth();
